...
e.g., `Exon=3` -or- `Exon=c(3,5,6)`

A single numeric or numeric vector that defines exons to target in the amino acid analysis. The amino acid alignment is parsed according to the overlap of the defined exons. Only relevant to the amino acid analysis. The defined exons are not required to be continuous. Multiple sets are not analyzed separately. The defined exons are applied to all loci in the the analysis. If an exon does not exist for a given locus, BIGDAWG will register and error and the analysis will stop. In such instances, it is recommended you anlayze those Loci separately.
...
For the amino acid analysis, BIGDAWG is bundled with HLA protein alignment data using the above indicated database release. These bundled alignments can be updated to the most recent release [IMGT/HLA](https://www.ebi.ac.uk/ipd/imgt/hla). Future database updates do not guarantee compatibility with the updating tool.
